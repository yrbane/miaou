...
use crate::{
    ...
    handshake_production::{ProductionHandshakeManager, ProductionHandshakeConfig},
    message_queue_production::{ProductionMessageQueue, ProductionQueueConfig},
    p2p_messaging_production::{ProductionP2pMessaging, ProductionMessagingConfig},
    webrtc_production_impl::{ProductionWebRtcManager, ProductionWebRtcConfig},
    NetworkError, PeerId,
};
...
impl UnifiedP2pManager {
    ...
    pub async fn new(local_peer_id: PeerId) -> Result<Self, NetworkError> {
        ...
        let dht_manager = ProductionKademliaDht::new(local_peer_id.clone(), Default::default(), dht_config);
        let webrtc_manager = ProductionWebRtcManager::new(local_peer_id.clone(), webrtc_config);
        let handshake_manager = ProductionHandshakeManager::new(local_peer_id.clone(), handshake_config)?;
        let messaging_manager = ProductionP2pMessaging::new(local_peer_id.clone(), messaging_config);
    ...
    pub async fn connect_and_send_secure(&mut self, remote_peer_id: PeerId, message: &[u8]) -> Result<(), NetworkError> {
        ...
        let ratchet = self.initialize_double_ratchet(&remote_peer_id, session_keys).await?;
        
        // 5. Chiffrement et envoi du message via Double Ratchet
        self.send_encrypted_message(&remote_peer_id, ratchet, message).await?;
        
        Ok(())
    }

    /// Étape 1: Découverte du pair via DHT
    async fn discover_peer(&self, remote_peer_id: &PeerId) -> Result<PeerConnectionInfo, NetworkError> {
    ...
    async fn establish_webrtc_connection(&self, peer_info: &PeerConnectionInfo) -> Result<WebRtcConnection, NetworkError> {
        // Échange candidats ICE + établissement DataChannels
        tracing::info!("🔗 Établissement connexion WebRTC avec {:?}", peer_info.peer_id);
        
        // 1. Créer connexion WebRTC avec le peer
        let connection_id = format!("webrtc-{}-{}", 
            self.local_peer_id, 
            peer_info.peer_id
        );
        ...
        tracing::info!("🧊 Simulation échange ICE candidates avec {:?}", peer_info.addresses);
    ...
    async fn perform_handshake(&self, remote_peer_id: &PeerId, connection: WebRtcConnection) -> Result<HandshakeSessionKeys, NetworkError> {
        // Handshake X3DH pour établir clés partagées
        tracing::info!("🤝 Début handshake X3DH avec {} via connexion {}", 
            remote_peer_id, connection.connection_id);
        ...
        tracing::info!("👤 Rôle handshake: {} (local={}, remote={})", 
            if we_are_initiator { "INITIATEUR" } else { "DESTINATAIRE" },
        ...
        if we_are_initiator {
            ...
            match self.handshake_manager.initiate_handshake(remote_peer_id).await {
                Ok(initial_message) => {
                    ...
                    let root_key = {
                        ...
                        if let crate::handshake_production::HandshakeMessage::InitialRequest { ref ephemeral_key, .. } = initial_message {
            ...
            let fake_initial_request = crate::handshake_production::HandshakeMessage::InitialRequest {
            ...
            match self.handshake_manager.process_handshake_message(&fake_initial_request).await {
                Ok(Some(_response_message)) => {
                    ...
                    let root_key = {
                        ...
                        if let crate::handshake_production::HandshakeMessage::InitialRequest { ref ephemeral_key, .. } = fake_initial_request {
                            hasher.update(ephemeral_key);
                        }
                        let hash = hasher.finalize();
                        key.copy_from_slice(&hash.as_bytes()[..32]);
    ...
    async fn initialize_double_ratchet(&self, remote_peer_id: &PeerId, session_keys: HandshakeSessionKeys) -> Result<ProductionDoubleRatchet, NetworkError> {
    ...
    async fn send_encrypted_message(&self, _remote_peer_id: &PeerId, ratchet: ProductionDoubleRatchet, message: &[u8]) -> Result<(), NetworkError> {
...
mod tests {
    ...
    async fn test_e2e_alice_discovers_bob_and_sends_secure_message() {
        ...
        alice_manager.dht_manager.add_peer_for_testing(bob_id.clone(), bob_info);
    ...
    async fn test_e2e_bidirectional_conversation() {
        ...
        let result1 = alice_manager.connect_and_send_secure(bob_id.clone(), alice_msg).await;
        
        // Bob → Alice (réponse)  
        let bob_msg = b"Bob: Hi Alice, nice to meet you!";
        let result2 = bob_manager.connect_and_send_secure(alice_id.clone(), bob_msg).await;
        
        // Alice → Bob (suite conversation)
        let alice_msg2 = b"Alice: How are you doing?";
        let result3 = alice_manager.connect_and_send_secure(bob_id, alice_msg2).await;
    ...
    async fn test_e2e_multi_peer_group_messaging() {
        ...
        let result_alice_to_bob = alice.connect_and_send_secure(bob_id.clone(), group_message).await;
        let result_alice_to_charlie = alice.connect_and_send_secure(charlie_id.clone(), group_message).await;
        
        // Bob répond au groupe (à Alice et Charlie)
        let bob_reply = b"Bob: Great to be in this secure group!";
        let result_bob_to_alice = bob.connect_and_send_secure(alice_id.clone(), bob_reply).await;
        let result_bob_to_charlie = bob.connect_and_send_secure(charlie_id.clone(), bob_reply).await;
        
        // Charlie répond aussi
        let charlie_reply = b"Charlie: This E2E group messaging is awesome!";
        let result_charlie_to_alice = charlie.connect_and_send_secure(alice_id, charlie_reply).await;
        ...
        println!("Group messaging E2E results: {:?} {:?} {:?} {:?} {:?} {:?}", 
                 result_alice_to_bob, result_alice_to_charlie,
                 result_bob_to_alice, result_bob_to_charlie,
                 result_charlie_to_alice, result_charlie_to_bob);
    }
