...
impl ProductionDataChannel {
    ...
    pub async fn new(label: String) -> Result<Self, NetworkError> {
        ...
        info!(
            "🔗 Data Channel production créé: {} sur {}",
            label,
            ...
            match tokio::time::timeout(Duration::from_secs(5), socket.recv(&mut buffer)).await {
                ...
                Err(_) => {
                    ...
                    Err(NetworkError::General(
                        "Timeout handshake WebRTC".to_string(),
                    ))
                }
            }
        } else {
            Err(NetworkError::General(
                "Socket UDP non initialisé".to_string(),
            ))
        }
    }
    ...
    pub async fn send(&self, data: &[u8]) -> Result<(), NetworkError> {
        if self.state != RealConnectionState::Connected {
            return Err(NetworkError::General(
                "Data Channel non connecté".to_string(),
            ));
        }
        ...
        if let Some(socket) = &self.udp_socket {
            ...
            Err(NetworkError::General(
                "Socket UDP non disponible".to_string(),
            ))
        }
    }
    ...
    pub async fn recv(&self) -> Result<Vec<u8>, NetworkError> {
        ...
        if let Some(message) = buffer.pop() {
            ...
            Err(NetworkError::General(
                "Pas de données disponibles".to_string(),
            ))
        }
    }
...
mod tests {
    ...
    async fn test_data_channel_local_communication() {
        ...
        let mut channel1 = ProductionDataChannel::new("sender".to_string())
            .await
            .unwrap();
        let mut channel2 = ProductionDataChannel::new("receiver".to_string())
            .await
            .unwrap();
