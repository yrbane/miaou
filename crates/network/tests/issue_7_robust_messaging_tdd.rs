...
use miaou_network::{
    AckStatus, AcknowledgableMessage, Message, MessageAck, PeerId, RobustMessagingConfig,
    RobustMessagingManager,
};
...
async fn test_tdd_red_stable_id_deduplication() {
    ...
    assert_eq!(
        ...
        "Un duplicata devrait être détecté"
    );
    assert_eq!(
        stats.messages_received, 1,
        "Un seul message devrait être compté comme reçu"
    );
...
async fn test_tdd_red_end_to_end_acknowledgments() {
    ...
    let message_id = sender_manager
        .send_with_guarantees(message.clone(), true)
        .await;
    assert!(message_id.is_ok());
    ...
    assert!(
        received.unwrap(),
        "Message devrait être accepté par le destinataire"
    );
    ...
    assert_eq!(
        ...
        "Un message devrait être confirmé envoyé"
    );
...
async fn test_tdd_red_load_test_100_messages() {
    ...
    assert!(
        load_test_result.is_ok(),
        "Test de charge ne devrait pas timeout"
    );
    ...
    assert_eq!(
        load_results.total_sent, message_count,
        "100 messages devraient être envoyés"
    );
    assert!(
        elapsed < Duration::from_secs(60),
        "Traitement devrait prendre < 60s"
    );
...
async fn test_tdd_red_duplicate_ack_handling() {
    ...
    assert!(
        ...
        "Au moins un ACK devrait être compté"
    );
...
async fn test_tdd_red_message_expiration_cleanup() {
    ...
    assert!(
        cleaned >= 1,
        "Au moins un message expiré devrait être nettoyé"
    );
...
async fn test_tdd_red_issue_7_complete_integration() {
    ...
    for (i, msg_id) in message_ids.iter().enumerate() {
        ...
        assert!(
            !result2.unwrap(),
            "Duplicata message {} devrait être rejeté",
            i + 1
        );
    ...
    info!(
        ...
        final_stats.acks_sent,
        final_stats.acks_received,
        final_stats.duplicates_detected
    );

    // Vérifications finales Issue #7
    assert_eq!(
        final_stats.messages_sent, 5,
        "5 messages devraient être confirmés envoyés"
    );
    assert_eq!(
        final_stats.messages_received, 5,
        "5 messages devraient être reçus"
    );
    ...
    assert_eq!(
        final_stats.duplicates_detected, 5,
        "5 duplicatas devraient être détectés"
    );
