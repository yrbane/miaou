...
use miaou_network::{PeerId, PeerInfo, Transport, TransportConfig, WebRtcTransport};
...
async fn test_webrtc_real_datachannel_e2e() {
    ...
    let _peer1 = PeerInfo::new(PeerId::from_bytes(b"webrtc-test-peer1".to_vec()));
    ...
    match connection_result {
        Ok(Ok(connection)) => {
            ...
            if connection_latency < Duration::from_millis(5000) {
                // Plus généreux pour CI
                info!("🟢 Latence acceptable pour E2E: {:?}", connection_latency);
            } else {
                debug!(
                    "⚠️  Latence élevée (acceptable en CI): {:?}",
                    connection_latency
                );
            }
