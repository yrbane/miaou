...
mod net_connect_tdd_tests {
    ...
    async fn test_net_connect_command_parsing() {
        ...
        assert!(result.is_err(), "Should fail in RED phase - parsing not implemented");
    }
    ...
    async fn test_net_connect_with_valid_peer_id() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                execute_net_connect_command(command).await
            })
        }));
        
        // Phase RED: on s'attend à un échec car pas implémenté
        assert!(result.is_err(), "Should fail in RED phase - net-connect not implemented");
    }
    ...
    async fn test_net_connect_with_invalid_peer_id() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                validate_peer_id(&invalid_peer_id)
            })
        }));
        
        // Phase RED: validation pas implémentée
        assert!(result.is_err(), "Should fail in RED phase - validation not implemented");
    }
    ...
    async fn test_net_connect_integration_with_discovery() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                connect_to_discovered_peer(peer_id).await
            })
        }));
        
        // Phase RED: intégration découverte pas implémentée
        assert!(result.is_err(), "Should fail in RED phase - discovery integration not implemented");
    }
    ...
    async fn test_net_connect_shows_connection_status() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                show_connection_status(peer_id).await
            })
        }));
        
        // Phase RED: affichage statut pas implémenté
        assert!(result.is_err(), "Should fail in RED phase - status display not implemented");
    }
    ...
    fn test_net_connect_command_struct() {
        ...
        let command = Command::NetConnect { peer_id: peer_id.clone() };
        
        // Vérifier que la commande capture les bonnes données
        match command {
            Command::NetConnect { peer_id: captured_id } => {
    ...
    async fn test_command_executor_pattern() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                execute_command_with_p2p_manager(command).await
            })
        }));
        
        // Phase RED: executor pattern pas implémenté
        assert!(result.is_err(), "Should fail in RED phase - executor pattern not implemented");
    }
    ...
    async fn test_connection_timeout_handling() {
        ...
        assert!(result.is_err(), "Should fail in RED phase - timeout handling not implemented");
    }
    ...
    async fn test_net_connect_uses_mdns_discovered_peers() {
        ...
        assert!(result.is_err(), "Should fail in RED phase - mDNS integration not implemented");
    }
    ...
    async fn test_net_connect_peer_not_discovered() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                handle_peer_not_found(unknown_peer_id).await
            })
        }));
        
        // Phase RED: gestion pair non trouvé pas implémentée
        assert!(result.is_err(), "Should fail in RED phase - peer not found handling not implemented");
    }
    ...
    async fn connect_with_timeout(_peer_id: &str, _timeout: std::time::Duration) -> Result<(), String> {
        // RED: Pas implémenté - va panic
...
mod solid_integration_tests {
    use miaou_network::p2p_connection::{P2pConnectionManager, MockP2pConnectionFactory, MockHandshakeProtocol};
    ...
    async fn test_cli_integrates_with_p2p_connection_manager() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            tokio::runtime::Handle::current().block_on(async {
                cli_uses_p2p_manager(&p2p_manager, "target-peer").await
            })
        }));
        
        // Phase RED: intégration pas implémentée
        assert!(result.is_err(), "Should fail in RED phase - CLI P2P integration not implemented");
    }
    ...
    async fn test_net_connect_with_dependency_injection() {
        ...
        assert!(result.is_err(), "Should fail in RED phase - dependency injection not implemented");
    }

    // Fonctions manquantes pour tests d'intégration SOLID
    async fn cli_uses_p2p_manager(_manager: &P2pConnectionManager, _peer_id: &str) -> Result<(), String> {
