...
mod tests {
    ...
    fn test_encryption_logic_secure_prefix() {
        ...
        for (dest, should_be_secure) in destinations {
            ...
            if is_secure {
                ...
                assert!(!actual_peer.is_empty() || dest == "secure:", "Should extract peer");
            }
        }
    }
    ...
    fn test_message_limit_constant() {
        ...
        assert!(MAX_MESSAGES > 0, "Message limit should be positive");
        assert!(MAX_MESSAGES <= 1000, "Message limit should be reasonable");
    }
    ...
    async fn test_diagnostics_real_stun_tests() {
        ...
        assert!(duration.as_secs() < 30, "Diagnostics should complete within 30s");
    }
    ...
    async fn test_verify_returns_error_on_failure() {
        ...
        if let Err(MiaouError::Crypto(msg)) = result {
            assert!(msg.contains("verification failed"), "Should have verification error");
        } else {
    ...
    fn test_init_tracing_no_panic_on_reinit() {
        ...
        assert!(true, "init_tracing should not panic on reinit");
    }
    ...
    async fn test_encryption_key_derivation() {
        ...
        let session_key = miaou_crypto::blake3_hash(
            format!("session_{}_{}", "local_peer", peer_id).as_bytes()
        );
        ...
        let session_key2 = miaou_crypto::blake3_hash(
            format!("session_{}_{}", "local_peer", peer_id).as_bytes()
        );
        assert_eq!(session_key, session_key2, "Key derivation should be deterministic");
    }
    ...
    fn test_recv_loop_logic() {
        ...
        assert!(received_count <= max_messages, "Should respect message limit");
        assert!(received_count < max_messages, "Should exit before limit in normal case");
    }
    ...
    async fn test_chacha20poly1305_encryption_basic() {
        ...
        assert!(ciphertext.len() > plaintext.len(), "Ciphertext includes auth tag");
    ...
    async fn test_history_help_message_content() {
        ...
        let cli = Cli {
            log: "error".to_string(), 
            json: false,
            cmd: Command::History { limit: 1, peer: Some("alice".to_string()) },
        };
