...
ensure_label() {
  ...
  # Upsert en une commande (évite les warnings) :
  gh label create "$name" --repo "$REPO" --description "$desc" --color "$color" --force >/dev/null 2>&1 || true
}
...
add_labels() {
  local num="$1"; shift
  for lb in "$@"; do
    gh issue edit "$num" --repo "$REPO" --add-label "$lb" >/dev/null
  done
}
...
# ---------- Decisions (single-quoted titles) ----------
CLOSE_OK_TITLES=(
...
Référence (branche): https://raw.githubusercontent.com/yrbane/miaou/v0.2.0-radar-moustaches/docs/versions/v0.2.0-radar-moustaches.md
BODY
        ;;
      '[WebRTC] Data Channels réels (offer/answer + ICE)')
        edit_body_from_stdin "$num" <<'BODY'
